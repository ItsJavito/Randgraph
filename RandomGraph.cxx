...
void random_bullseye(int n, int d, ofstream &out) {
  ...
  for(i=0; i<orbits; i++) {
    ...
    for(j=0; j<per_orbit * d / 2; j++) {
      endpt1 = (rand() % per_orbit) + i * per_orbit;
      endpt2 = (rand() % per_orbit) + i * per_orbit;
      weight = base + scale * (rand() / (RAND_MAX + 1.0));
	...
	endpt1 = (rand() % per_orbit) + i * per_orbit;    
	endpt2 = (rand() % per_orbit) + (i-1) * per_orbit;
	weight = base + scale * (rand() / (RAND_MAX + 1.0));
	out << endpt1 << " " << endpt2 << " " << weight << endl;
      }
...
void random_hier(int lo, int hi, int edges, double scale, ofstream &out) {
  ...
  for(i=0; i<edges; i++) {
    endpt1 = rand() % range + lo;
    endpt2 = rand() % range + lo;
    weight = scale + (rand() / (RAND_MAX + 1.0)) * scale;
    out << endpt1 << " " << endpt2 << " " << weight << endl;
  }
}
...
void random_Gnp(int n, int density, ofstream &out) {
  ...
  for(i=0;i<n;i++)
    for(j=i+1;j<n;j++)
      if((rand() / (RAND_MAX + 1.0)) <= p) {     // generate (i,j) with prob. p
	weight = random_weight();
	out << i << " " << j << " " << weight << "\n";
      }
  out << "-1";
...
void random_2dtorus(int n, int pct, ofstream &out) {
  ...
  for(i=0; i<n; i++) 
    for(j=0; j<n; j++) {
      if((rand() / (RAND_MAX + 1.0)) <= p) {    // add edge going from (i,j) to (i+1,j)
	weight = random_weight();
	out << i*n + j << " " << ((i+1) % n)*n + j << " " << weight << endl;
      }
      if((rand() / (RAND_MAX + 1.0)) <= p) {    // add edge going from (i,j) to (i,j+1)
	weight = random_weight();
...
void random_3dtorus(int n, int pct, ofstream &out) {
  ...
  for(i=0; i<n; i++) 
    for(j=0; j<n; j++)
      for(k=0; k<n; k++) {
	if((rand() / (RAND_MAX + 1.0)) <= p) {    // add edge going from (i,j,k) to (i+1,j,k)
	  weight = random_weight();
	  out << i*n*n           + j*n + k << " " 
	      << ((i+1) % n)*n*n + j*n + k << " " 
	      << weight << endl;
	}
	if((rand() / (RAND_MAX + 1.0)) <= p) {    // add edge going from (i,j,k) to (i,j+1,k)
	  weight = random_weight();
	  out << i*n*n + j*n           + k << " " 
	      << i*n*n + ((j+1) % n)*n + k << " " 
	      << weight << endl;
	}
	if((rand() / (RAND_MAX + 1.0)) <= p) {    // add edge going from (i,j,k) to (i,j,k+1)
	  weight = random_weight();
	  out << i*n*n + j*n + k       << " " 
	      << i*n*n + j*n + (k+1)%n << " " 
	      << weight << endl;
